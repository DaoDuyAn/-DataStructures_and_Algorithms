...
int kiemTraCNPTK(Nut *T) {
    ...
    if (T->Left != NULL and Max_BST(T->Left) > T->Info) return 0;

    if (T->Right != NULL and Min_BST(T->Right) < T->Info) return 0;

    if (kiemTraCNPTK(T->Left) == 0 or kiemTraCNPTK(T->Right) == 0) return 0;
